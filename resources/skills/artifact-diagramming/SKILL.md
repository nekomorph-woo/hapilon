...
description: Diagramming know-how for standalone HTML pages and reports — when a picture earns its place, how to draw one that shows the real mechanism, and the inline-SVG mechanics that keep it legible in both themes.
---
...
These mechanics apply to the self-contained HTML pages we deliver, where SVG is always inlined: hand-author inline `<svg>` with native shapes (`rect`, `circle`, `line`, `polyline`, `path`) and `<text>` — no libraries, no runtime, no external images.
