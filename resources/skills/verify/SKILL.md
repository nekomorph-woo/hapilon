...
**Check `.pi/skills/` first — even if you already know how to
...
usually `apps/desktop/.pi/skills/`, not the root. Probe both:

```bash
ls .pi/skills/                    # repo root
ls <touched-dir>/.pi/skills/      # each dir level the diff names
```

- **`verifier-*` matching your surface** (CLI verifier for a CLI
  change, etc.) → invoke it via `/skill:<name>` or read its SKILL.md
  and follow it. Mismatched surface → skip that one, try the next. Stale
...
- **Neither** → cold start from README/package.json/Makefile. Timebox
  ...
  prompt describing the build/launch/drive recipe a project
  verify/run skill would need to capture. Got through → **persist
  what you learned**: create `.pi/skills/verify/SKILL.md` at the
  level you probed above — repo root for a single-package repo; the
  touched package/app dir (`apps/desktop/.pi/skills/verify/SKILL.md`) in
...
**Evidence has to reach the reader.** Evidence has to reach the
reader: pane captures and response bodies belong inline in the
report. Only reference a file path when the reader shares your
filesystem.
...
- **BLOCKED** — couldn't reach a state where the change is observable.
  ...
  stopped + a prompt describing the build/launch/drive recipe a
  project verify/run skill would need to capture.
