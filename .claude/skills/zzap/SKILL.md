...
- **普通提交**：z+1（例如 `0.1.0` → `0.1.1`）
- **版本发布-普通**：y+1, z归零（例如 `0.5.30` → `0.6.0`）
- **版本发布-超大规模**：x+1, y.z归零（例如 `1.33.2` → `2.0.0`）
- 版本文件含 `v` 前缀时，写回保留前缀；CHANGELOG/PR/TAG 中取纯数字部分
- 版本号提取正则：`(\d+\.\d+\.\d+)`
- **hapilon 专属**：当前版本 `0.1.0-alpha`，`-alpha` 后缀始终保留，只升级前面的数字部分。版本计算时忽略后缀，写回时拼回（例如 `0.1.1-alpha`）
