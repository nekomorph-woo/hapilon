...
import {
  ...
  splitSimpleCommandsWithSeparators,
...
interface VarValue {
  /** mktemp 产物（无具体路径）；path 为已展开的字面路径；unknown 表示无法静态解析 */
  kind: "mktemp" | "path" | "unknown";
  path?: string;
}
...
function dequote(view: string): string {
  let out = "";
  let i = 0;
  while (i < view.length) {
    const ch = view[i]!;
...
function commandWordTokenIndex(tokens: Token[]): number {
  let i = 0;
...
function analyzeMktemp(body: string, opts: SandboxOptions): VarValue {
  const tokens = body.trim().split(/\s+/).filter(Boolean);
  if (tokens.length === 0) return { kind: "unknown" };
  const word = tokens[0]!.split("/").pop() ?? "";
  if (word !== "mktemp") return { kind: "unknown" };
  ...
  if (template === undefined) return { kind: "mktemp" };
  const expanded = expandRefs({ text: template, literal: false }, new Map(), opts);
  if (expanded.unknown) return { kind: "unknown" };
...
function expandRefs(
  ...
  opts: SandboxOptions,
): { text: string; unknown: boolean; hasMktemp: boolean } {
  if (token.literal) return { text: token.text, unknown: false, hasMktemp: false };
...
function normalizePath(path: string, opts: SandboxOptions): string {
...
function resolveTargetToken(token: Token, vars: Map<string, VarValue>, opts: SandboxOptions): SandboxTarget {
  ...
  if (expanded.hasMktemp) {
    ...
    const escaped =
      !expanded.text.startsWith(MKTEMP_MARK) || expanded.text.split("/").includes("..");
    return {
      raw: token.text,
      resolved: "(mktemp)",
      sandboxed: !escaped,
...
function resolveCdTarget(
  ...
  for (let i = wordIdx + 1; i < tokens.length; i++) {
    const t = tokens[i]!;
    if (!dashDash && t.text === "-") return undefined; // cd - 用 OLDPWD，静态不可知
    if (!dashDash && t.text === "--") {
      dashDash = true;
      continue;
    }
    if (!dashDash && !t.literal && t.text.startsWith("-") && t.text.length > 1) continue; // -P/-L/-e
    if (t.text.includes(SUB_PLACEHOLDER)) return undefined;
    // 上一个 cd 已不可解析：相对 cd 目标同属不可判
    if (opts.cwd === UNKNOWN_CWD && !isAbsolute(t.text)) return undefined;
    const expanded = expandRefs(t, vars, opts);
    if (expanded.unknown || expanded.hasMktemp || expanded.text === "") return undefined;
...
function destructiveWord(word: string, rest: Token[]): boolean {
  if (word === "rm" || word === "chmod" || word === "chown") return true;
  return word === "sed" && rest.some((t) => !t.literal && t.text.startsWith("-i"));
}

/** 单个简单命令的写目标（破坏性命令词的操作数 + 重定向目标） */
function segmentTargets(tokens: Token[]): Token[] {
  const targets: Token[] = [];
    ...
    if (destructiveWord(word, rest)) {
      if (word === "sed") {
        ...
        for (const t of rest) {
          ...
          if (!dashDash && !t.literal && t.text.startsWith("-") && t.text.length > 1) continue;
          if (t.text === "") continue; // macOS `sed -i ""` 的空备份后缀不是文件目标
          if (skipScript) {
            skipScript = false;
            continue;
          }
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (/^(1|2|&)?>>?$/.test(t.text)) {
      ...
      if (next && !next.text.startsWith("&")) targets.push(next);
      continue;
    }
    const glued = /^(?:1|2|&)?>>?(?!<)(.+)$/.exec(t.text);
    if (glued && !glued[1]!.startsWith("&")) targets.push({ text: glued[1]!, literal: false });
  }
  return targets;
}
 ...
 */
function cwdAfterCd(
  cdResult: string | undefined,
  sep: SegmentSeparator | undefined,
  prevSep: SegmentSeparator | undefined,
  cwdBefore: string,
  andOrStartCwd: string,
): string {
  if (sep === "&") return andOrStartCwd;
  if (sep === "|" || prevSep === "|" || sep === "subshell" || prevSep === "subshell") return cwdBefore;
  if (sep === "&&") return cdResult ?? UNKNOWN_CWD;
  return UNKNOWN_CWD;
}
 ...
 */
export function checkSandboxWrite(command: string, opts: SandboxOptions): SandboxCheck {
  ...
  let prevSep: SegmentSeparator | undefined;
  ...
  for (const { text, sep } of segments) {
    ...
    const segOpts: SandboxOptions = { ...opts, cwd };
    // 前导赋值：VAR=…（值可为替换占位 / 字面路径 / 引用已有变量）
    for (; i < tokens.length; i++) {
      const t = tokens[i]!;
      if (!isAssignment(t)) break;
      ...
      if (placeholderCount > 0) {
        ...
        vars.set(
          t.text.slice(0, t.text.indexOf("=")),
          placeholderCount === 1 && value === SUB_PLACEHOLDER
    ...
    for (; i < tokens.length; i++) {
      if (tokens[i]!.text === SUB_PLACEHOLDER && !tokens[i]!.literal) bodyIndex++;
    }
    const wordIdx = commandWordTokenIndex(tokens);
    const word = wordIdx >= 0 ? tokens[wordIdx]!.text : "";
    if (destructiveWord(word, tokens.slice(wordIdx + 1))) hasDestructive = true;
