...
export function stripQuotedText(command: string): string {
  let out = "";
  let i = 0;
...
export function extractSubstitutions(command: string): { view: string; bodies: string[] } {
  const bodies: string[] = [];
  let out = "";
  let i = 0;
  while (i < command.length) {
...
export function commandWordAt(simpleCommand: string): { word: string; index: number } {
  let offset = 0;
  ...
  for (;;) {
    ...
    if (!m || !PREFIX_WORDS.has(m[1]!)) break;
    offset += m[0].length;
    rest = rest.slice(m[0].length);
  }
  const m = /^\s*(\S+)/.exec(rest);
  if (!m) return { word: "", index: simpleCommand.length };
 ...
 */
export function extractShellPayloads(command: string): string[] {
...
export function substitutionInDestructiveTarget(subView: string): boolean {
  const sub = SUB_PLACEHOLDER;
  return (
    new RegExp(`\\brm\\s+-rf\\b[^\\n]*?["']?\\s*${sub}`).test(subView) ||
