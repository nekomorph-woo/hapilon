...
export class PanelViewer extends FloatingPane {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  constructor(tui: any, theme: Theme, _kb: unknown, done: () => void, target?: any) {
    super(tui, theme, _kb, done, { title: "", lines: ["loading..."] } as FloatingPaneOptions);
    this.tuiInst = tui;
  ...
  sync(): void {
    ...
    this.titles = this.panels.map((p: any) => panelTitle(p, this.tuiInst.terminal.columns));
  ...
  render(width: number): string[] {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    this.visibleRows = Math.max(6, Math.floor(((this.tuiInst as any).terminal?.rows ?? 40) * 0.85) - 4);

    const border = (c: string) => th.fg("border", c);
    const row = (s: string) => border("│") + truncateToWidth(s, this.innerW, "…", true) + border("│");
    ...
    for (const line of raw) {
      ...
      else for (const p of parts) content.push(p);
    }
    ...
    for (let i = slice.length; i < this.visibleRows; i++) out.push(row(""));
    ...
    out.push(
      border("│") +
        th.fg("dim", truncateToWidth(
          ` ←/→ panel   ↑↓ scroll   shift+↑↓ page   esc close${range}`,
          this.innerW, "…", true,
        )) +
        border("│"),
    );
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function launchViewer(ui: any, target?: any): void {
  if (state.viewerOpen || typeof ui?.custom !== "function") return;
  state.viewerOpen = true;
  ui.custom(
    (tui: unknown, theme: Theme, _kb: unknown, done: () => void) =>
      new PanelViewer(tui, theme, _kb, done, target),
    {
      overlay: true,
