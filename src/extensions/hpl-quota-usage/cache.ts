...
import { quotaNamespace, type QuotaSnapshot, type QuotaSnapshotFile } from "./snapshot.js";
 ...
 */
function readSnapshotMap(path: string): Record<string, QuotaSnapshot> {
...
export function readQuotaSnapshots(now = Date.now()): QuotaSnapshot[] {
...
export function readQuotaSnapshotFor(provider: string, now = Date.now()): QuotaSnapshot | undefined {
...
export function writeQuotaSnapshot(snapshot: QuotaSnapshot): void {
  try {
    ...
    const now = Date.now();
    const providers: Record<string, QuotaSnapshot> = {};
    for (const [key, value] of Object.entries(readSnapshotMap(path))) {
      if (!isStale(value, now)) providers[key] = value;
    }
    providers[snapshot.provider] = snapshot;
    const file: QuotaSnapshotFile = { version: 2, providers };
