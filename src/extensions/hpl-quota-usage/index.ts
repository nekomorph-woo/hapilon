...
import {
  ...
  parseSnapshot as parseGlmSnapshot,
...
function snapshotKey(provider: string): string {
  return provider === "zai" || provider === "zai-coding-cn" ? "glm" : provider;
}

function fetchAndParseSnapshot(provider: string, auth: QuotaAuth, now: number): Promise<QuotaSnapshot> {
  const key = snapshotKey(provider);
  const run = (effect: Effect.Effect<unknown, Error>): Promise<QuotaSnapshot> =>
    Effect.runPromise(effect).then((payload) => {
      switch (key) {
        case "deepseek":
...
let lastAuth: QuotaAuth | undefined;
let refreshInFlight = false;

export function refreshCache(provider: string, auth: QuotaAuth, now = Date.now()): void {
  if (refreshInFlight) return;
  refreshInFlight = true;
  fetchAndParseSnapshot(provider, auth, now)
    .then(
      (snapshot) => writeQuotaSnapshot({ ...snapshot, provider: snapshotKey(provider) }),
      () => {},
    )
    .finally(() => {
...
export function ensurePolling(provider: string, auth: QuotaAuth): void {
  lastProvider = provider;
  lastAuth = auth;
  refreshCache(provider, auth);
  if (pollTimer) return;
  pollTimer = setInterval(() => {
    if (lastProvider && lastAuth) refreshCache(lastProvider, lastAuth);
  }, POLL_INTERVAL_MS);
  pollTimer.unref?.();
}

export function stopPollingForTest(): void {
...
export default function hplQuotaUsage(pi: ExtensionAPI): void {
  ...
  const startPollingFor = (ctx: { model?: Model<any> } & ExtensionCommandContext): void => {
    const model = ctx.model;
    if (!model || !isSupportedProvider(model.provider)) return;
    void (async () => {
      const auth = await Effect.runPromise(resolveAuthEffect(ctx as ExtensionCommandContext, model));
      if (auth) ensurePolling(model.provider, auth);
    })();
  };

  pi.on("session_start", (_event, ctx) => {
    startPollingFor(ctx as typeof ctx & ExtensionCommandContext);
  });
  ...
  pi.registerCommand("quota-usage", {
    ...
    handler: async (_args, ctx) => {
      ...
      const SPACER = field("", " ");
      const renderLine = (item: QuotaField): string =>
        item.value === SPACER.value && item.label === "" ? "" : item.label ? `  ${item.label}: ${item.value}` : item.value;
      ...
      const configured = new Map<string, QuotaAuth>();
      for (const provider of providers) {
        ...
        if (auth) configured.set(provider, auth);
      }
      ...
      if (configured.size === 0) {
        ...
        const ordered = [...configured.keys()].sort((a, b) =>
          a === currentProvider ? -1 : b === currentProvider ? 1 : a.localeCompare(b));
        await Promise.all(ordered.map(async (provider) => {
          const result = await Effect.runPromise(queryQuotaEffect(provider, configured.get(provider)!).pipe(
          ...
          if (provider === currentProvider) refreshCache(provider, configured.get(provider)!);
