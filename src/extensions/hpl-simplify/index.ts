...
export default function hplSimplify(pi: ExtensionAPI): void {
  pi.registerCommand("simplify", {
    ...
    handler: async (args: string, ctx) => {
      ...
      if (!scope) {
        ctx.ui?.notify?.(
          "Usage: /simplify [ref | A..B | --staged] or /simplify apply <numbers>\n" +
            "  /simplify            audit the latest commit\n" +
            "  /simplify --staged   audit staged changes\n" +
            "  /simplify main..HEAD audit a range",
          "error",
        );
        return;
      }
      ...
      ctx.ui?.notify?.(
        "Read-only audit dispatched. Review the numbered report, then: /simplify apply <numbers>",
        "info",
      );
    },
  });
