...
export function buildTransientRolePrompt(tier: "opus" | "sonnet" | "haiku"): string {
...
export function buildConstraintFrame(userPrompt: string): string {
...
export function parseRoleDefSentinel(text: string): TeamRoleDef | undefined {
  for (const candidate of findJsonObjects(text)) {
    ...
    if (typeof value.key !== "string" || !ROLE_KEY_PATTERN.test(value.key)) continue;
    if (typeof value.label !== "string" || value.label.trim().length === 0) continue;
    if (typeof value.prompt !== "string" || value.prompt.trim().length === 0) continue;
    if (typeof value.tier !== "string" || !TIERS.has(value.tier)) continue;
    const tier = value.tier as "opus" | "sonnet" | "haiku";
    return {
      key: value.key,
