import { ROLE_KEY_PATTERN, type TeamRoleDef } from "./role-registry.js";
...
export function parseRoleDefSentinel(
  text: string,
  fallbackTier: "opus" | "sonnet" | "haiku" = "sonnet",
): TeamRoleDef | undefined {
  for (const candidate of findJsonObjects(text)) {
    ...
    const tier = typeof value.tier === "string" && TIERS.has(value.tier)
      ? (value.tier as "opus" | "sonnet" | "haiku")
      : fallbackTier;
    return {
      key: value.key,
