 ...
 *   第1行  工作目录 | 分支
 *   第2行  up.N | down.N | hit: N% | ctx/win: N%/W | [DING]     模型名 • thinking 档位
 *   第3行  扩展状态（存在时）
 *
 * [DING] 为上下文占用指示灯：背景色随占用率渐变 + 感叹号分级（见 ding.ts）。
    ...
    ctx.ui.setFooter((tui, theme, footerData) => {
      ...
      return {
        ...
        render(width: number): string[] {
          ...
          // 分段 dim：[DING] 自带真彩码且以复位结尾，整体包裹会被复位打断。
          // left 内 [DING] 位于末尾，其复位不影响 left 前段；余下部分单独 dim。
