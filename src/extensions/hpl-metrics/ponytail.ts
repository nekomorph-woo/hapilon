...
function groupKeyOf(sample: SessionSample, groupBy: GroupBy): string {
  switch (groupBy) {
    case "ponytail":
...
export function groupSamples(samples: SessionSample[], groupBy: GroupBy): MetricGroup[] {
  ...
  for (const [key, bucket] of buckets) {
    const sum = (pick: (sample: SessionSample) => number): number =>
      bucket.reduce((total, sample) => total + pick(sample), 0);
    groups.push({
      key,
...
export interface ReportMeta {
  ...
  groupBy: GroupBy;
  samples: number;
  /** 档位来自会话内记录的样本数（其余为默认推测） */
  ponytailFromSession: number;
}

/** 渲染成交互面板可用的文本行（表 + 表尾说明） */
export function renderPonytailLines(groups: MetricGroup[], meta: ReportMeta): string[] {
  const header = ["分组", "会话", "out中位", "out合计", "成本$", "文本/代码", "编辑/会话", "文件/会话", "bash/会话", "子代理/会话"];
  const rows: string[][] = groups.map((group) => {
    const per = (value: number): string => (group.sessions === 0 ? "0" : (value / group.sessions).toFixed(1));
  ...
  if (rows.length === 0) lines.push("（没有符合条件的会话）");
  ...
  lines.push(
    `ponytail 档位：${meta.ponytailFromSession}/${meta.samples} 个会话取自会话内记录` +
