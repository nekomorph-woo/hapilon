...
const toStartupError = (error: unknown): StartupError =>
  new StartupError({ message: error instanceof Error ? error.message : String(error) });
 ...
 */
export function discoverHerdrPiExtension(userPiAgentDir: string | undefined): string | undefined {
...
export const prepareStartupEffect = (args: string[]): Effect.Effect<PiLaunchPlan, StartupError> => Effect.gen(function* () {
  ...
  if (!config.safetyNoticeShown && !isNonInteractive) {
    ...
    yield* writeHapilonConfigEffect({ ...config, safetyNoticeShown: true }).pipe(
  ...
  // 重新拉起时必须用 hapilon 自己的入口）。本模块编译后在 dist/cli/ 下，上一级即 dist/cli.js。
