...
import { snapshotHot, type QuotaSnapshot } from "../../extensions/hpl-quota-usage/snapshot.js";
...
describe("quota 链路：snapshot → cache → footer", () => {
  ...
  afterEach(() => {
    if (originalCache !== undefined) process.env["HAPILON_QUOTA_CACHE"] = originalCache;
    else delete process.env["HAPILON_QUOTA_CACHE"];
  ...
  it("glm snapshot 写缓存后 footer 拼出模板 A 段", () => {
    ...
    const segment = buildQuotaSegment(
      loaded.windows.map((w) => ({ percent: w.percent, window: w.window, resetAt: w.resetAt })),
      loaded.balanceCny,
      now,
    );
