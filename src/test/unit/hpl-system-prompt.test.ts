...
describe("assembleSystemPrompt", () => {
  ...
  it("正常路径: 全量 section（含 contextFiles/skills/append）与顺序断言", () => {
    ...
    assert.ok(
      result.indexOf("<role_commit_boundary>") > result.indexOf("<commit_discipline>") &&
        result.indexOf("<role_commit_boundary>") < result.indexOf("<pi_documentation>"),
      "role_commit_boundary 紧随 commit_discipline",
    );
    assert.ok(
...
describe("写入目标授权边界与提交权（真实 assemble）", () => {
  ...
  after(() => {
    ...
    if (prevRole === undefined) delete process.env.HAPI_ORCH_ROLE;
    else process.env.HAPI_ORCH_ROLE = prevRole;
  });

  /** 按会话形态真实组装：herdr 面板 + 角色 env，与运行时接线同路径。 */
  function assembleAs(session: "plain" | "owner" | "worker" | "reviewer"): string {
    if (session === "plain") {
      delete process.env.HERDR_ENV;
      delete process.env.HAPI_ORCH_ROLE;
    } else {
      process.env.HERDR_ENV = "1";
      if (session === "owner") delete process.env.HAPI_ORCH_ROLE;
      else process.env.HAPI_ORCH_ROLE = session;
    }
    return assembleSystemPrompt(opts);
  }

  function assembleTeamOwner(): string {
    process.env.HERDR_ENV = "1";
    delete process.env.HAPI_ORCH_ROLE;
