...
type CaseModel = {
  ...
  lifecycle: string;
  vps: Vp[];
  run: { status: string; ok: number; fail: string[]; results: Array<{ vp_id: string; status: string }> } | null;
};

// 跑 explorer 并从产物 HTML 里读回数据模型：判定内核没导出（explorer.mjs 是 CLI），
// 且真正要保证的是「写进 YAML 的 operator 一路走到视图里的 status/message」。
function explorerModel(dir: string, cases: string, runs?: Record<string, unknown>): CaseModel[] {
  ...
  return (JSON.parse(m[1].replace(/;\s*$/, "")) as { cases: CaseModel[] }).cases;
}
...
const OPERATOR_EXPECTED: Record<string, string> = {
  "VP-001": "PASS",
  "VP-002": "FAIL",
  "VP-003": "PASS",
  "VP-004": "PASS",
...
function operatorRun(): { byId: Map<string, CaseModel> } {
...
const vpOf = (caseId: string, vpId: string): Vp => {
...
const yamlLite = (await import(pathToFileURL(join(SCRIPTS, "yaml-lite.mjs")).href)) as {
  numEq: (a: unknown, b: unknown) => boolean;
  isUndecidable: (v: unknown) => boolean;
};
...
describe("golden-case · yaml-lite 对不可判定期望的拒判", () => {
  ...
  - id: CASE-202
    ...
    expect:
      mood: 余额扣得正确，用户体验良好
`;
    const cases = withTmp((dir) =>
      explorerModel(dir, CASES, {
