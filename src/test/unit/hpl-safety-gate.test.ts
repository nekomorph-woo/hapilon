...
after(() => {
  if (ORIGINAL_HAPILON_HOME === undefined) delete process.env.HAPILON_HOME;
  else process.env.HAPILON_HOME = ORIGINAL_HAPILON_HOME;
...
describe("hpl-safety-gate", () => {
  ...
  describe("Auto 判定依赖注入", () => {
    ...
    function ctxWithModelRegistry(onModelCall: () => void) {
      return {
        cwd: "/tmp",
        hasUI: false,
        modelRegistry: {
    ...
    before(() => {
      writeFileSync(
        ...
        JSON.stringify({ opus: [], sonnet: [], haiku: [{ provider: "zai", id: "glm-4.7" }] }),
      );
    });
