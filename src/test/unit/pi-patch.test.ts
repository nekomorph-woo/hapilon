...
describe("ensurePiPatch()", () => {
	...
	function fixturePath(rule: (typeof PATCH_RULES)[number], chunk = CHUNK_NAME): string {
	...
	function writeChunkFixture(path: string): void {
		const chunkRules = PATCH_RULES.filter((rule) => rule.signature);
		writeFileSync(
			path,
			chunkRules.map((rule) => Array(rule.occurrences).fill(rule.find).join("\n")).join("\n") + "\n",
		);
	}
	...
	before(() => {
		...
		for (const target of new Set(
			PATCH_RULES.filter((r) => !r.signature).map((r) => `${r.package ?? PI}::${r.file}`),
		)) {
	...
	it("Windows shell 修复落在后台任务插件（hapilon 依赖树，不在 pi 包里）", () => {
		const text = readFileSync(
			join(base, "node_modules", "@nklisch/pi-background-tasks", "extensions/background-tasks.ts"),
			"utf8",
		);
