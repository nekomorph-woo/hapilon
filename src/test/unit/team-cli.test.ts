...
describe("team-tasks 文件协议", { concurrency: false }, () => {
  it("追加写出的文件是 pi-tasks 形状，nextId 自增", () => {
    ...
    assert.equal(
      Effect.runSync(appendPendingTaskEffect(path, { paneId: workerPane, subject: "第二件", brief: "/tmp/dossier" }, () => 2000)),
      "2",
    );
    const raw = JSON.parse(readFileSync(path, "utf8")) as { nextId: number; tasks: Array<Record<string, unknown>> };
  ...
  it("既有文件形状不符 → 报错退出，文件原样不动", () => {
    ...
    if (result._tag === "Right") assert.fail("形状不符必须报错，不许硬写");
...
describe("hapi team-status", { concurrency: false }, () => {
  ...
  it("in_progress 超阈且缺回执 → stale", () => {
    ...
    const store = JSON.parse(readFileSync(path, "utf8")) as { tasks: Array<Record<string, unknown>> };
