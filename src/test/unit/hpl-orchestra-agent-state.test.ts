...
function makeSpawn(options: { alive?: boolean; shell?: boolean; statuses?: string[]; reads?: string[] } = {}) {
  ...
  const spawn: SpawnFn = (bin, args) => {
    ...
    if (args[0] === "pane" && args[1] === "get") {
      if (options.alive === false) return { status: 1, stderr: "pane not found" };
...
describe("hpl-orchestra agent state resolution", { concurrency: false }, () => {
  ...
  it("stale 必须有「在办任务」这个输入：超阈且有在办任务、回执缺席才成立", () => {
    const over = { herdrStatus: "idle" as const, expectedReportAgeMs: DEFAULT_STALE_THRESHOLD_MS };
