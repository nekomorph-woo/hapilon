...
function repo(packageJson: object = {}): string {
...
describe("inspectProjectEffect", () => {
  ...
  it("lockfile 按 npm > pnpm > yarn > bun 优先，单个分别识别", () => {
    for (const [file, expected] of [["package-lock.json", "npm"], ["pnpm-lock.yaml", "pnpm"], ["yarn.lock", "yarn"], ["bun.lock", "bun"]] as const) {
      const dir = repo({});
    ...
    const dir = repo({});
    for (const file of ["package-lock.json", "pnpm-lock.yaml", "yarn.lock", "bun.lockb"]) writeFileSync(join(dir, file), "");
    assert.equal(inspectProject(dir).packageManager, "npm");
    const empty = repo({});
    assert.equal(inspectProject(empty).packageManager, undefined);
    clean(dir); clean(empty);
  });
  ...
  it("greenfield 区分空扫描、lockfile、源码和扫描失败", () => {
    ...
    try { assert.equal(inspectProject(failed).isGreenfield, false); } finally { chmodSync(failed, 0o700); clean(failed); }
  });
  ...
  it("深层目录读取失败跳过并继续，不失败", () => {
    ...
    try {
      ...
      assert.equal(inspectProject(dir).effectImportsFound, true);
    } finally { chmodSync(blocked, 0o700); clean(dir); }
  });
});
