...
import {
  ...
  type ModelEvidence,
  type ProfileView,
} from "../../extensions/hpl-model-tiers/selector.js";

const cand = (
  provider: string,
  id: string,
  extra: { thinking?: CandidateModel["thinking"]; entryIndex?: number } = {},
): CandidateModel => ({
  provider,
  id,
...
const evidence = (overrides: Partial<ModelEvidence> = {}): ModelEvidence => ({
  trustedSamples: 0,
  explicitPicks: 0,
  switchPicks: 0,
  completedTasks: 0,
...
const profile = (models: Record<string, ModelEvidence>, roleLeaders: Record<string, string> = {}): ProfileView =>
  ({ models, roleLeaders });

/** 三个 sonnet 候选：同一模型名空间，只有 entryIndex 区分配置顺序 */
const trio = (): CandidateModel[] => [
...
describe("candidateLabels", () => {
  it("一次通过按审批率而非绝对次数", () => {
    const labels = candidateLabels(
      evidence({ trustedSamples: 6, approvals: 6, roles: { worker: 1 } }),
      "ok",
      profile({}),
      "worker",
      "zai/glm-5.3",
...
describe("thinking 偏好标签", () => {
  const withThinking = (levels: Record<string, Record<string, number>>): ProfileView =>
    profile({ "zai/glm-5.3": evidence({ thinkingLevels: levels }) });

  it("4 条未达门槛：显示「thinking 观察中」而不出具体 level", () => {
    const labels = candidateLabels(
      evidence({ thinkingLevels: { high: { worker: 4 } } }),
      "ok",
      withThinking({ high: { worker: 4 } }),
      "worker",
      "zai/glm-5.3",
    );
  ...
  it("5 条达门槛：显示固定文案「Worker 常用 thinking high」", () => {
    const labels = candidateLabels(
      evidence({ thinkingLevels: { high: { worker: 5 } } }),
      "ok",
      withThinking({ high: { worker: 5 } }),
      "worker",
      "zai/glm-5.3",
    );
