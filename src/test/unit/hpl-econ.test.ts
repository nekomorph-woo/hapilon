...
import hplEcon, { statsLine, setSessionDisabled, stats } from "../../extensions/hpl-econ/index.js";
...
function makeMockPi() {
  ...
  return {
    pi: {
      registerTool: (def: { name: string; execute: Function }) => tools.set(def.name, def),
...
describe("hpl-econ 扩展", () => {
  ...
  function bashEvent(text: string, details?: unknown) {
    return {
  ...
  it("默认开启：超阈值 bash 输出被压缩，返回省略提示与保留区", () => {
    ...
    const result = hook(bashEvent(lines)) as { content: Array<{ text: string }> } | undefined;
  ...
  it("setSessionDisabled(true)（--no-econ 映射）旁路；恢复后重新压缩", () => {
    ...
    const result = hook(bashEvent("b".repeat(20000))) as { content: Array<{ text: string }> };
  ...
  it("/econ 会话内切换 Disabled/Enabled 即时生效", async () => {
    ...
    resetVisit();
    await cmd.handler("", { ui, hasUI: true, mode: "tui" } as never);
    assert.equal(hook(bashEvent("c".repeat(20000))), undefined, "切换后旁路");
    // 第二次进入：toggle → Enabled → Close
    resetVisit();
    await cmd.handler("", { ui, hasUI: true, mode: "tui" } as never);
    const result = hook(bashEvent("d".repeat(20000))) as { content: Array<{ text: string }> };
  ...
  it("ctx_more 工具：按 ref 与行号取回省略片段", async () => {
    ...
    const res = (await tool.execute("t1", { ref: refMatch[1], from: 100, to: 104 })) as {
  ...
  it("ctx_more：未知 ref 返回 isError", async () => {
    ...
    const tool = mock.tools.get("ctx_more")!;
    const res = (await tool.execute("t1", { ref: "ghost", from: 1, to: 5 })) as { isError?: boolean };
  ...
  it("压缩全文落盘到 econ-store，内容 = 传给扩展的完整文本", () => {
    ...
    const files = readdirSync(store) as string[];
