...
import hplContext, {
  discoverBuiltInSkillsEffect,
  resolveHapilonSkillsDirEffect,
} from "../../extensions/hpl-context/index.js";
...
function resourceHandler(): (event: { cwd: string }) => { skillPaths: string[] } {
  const handlers: Array<(event: { cwd: string }) => { skillPaths: string[] }> = [];
  hplContext({
    on: ((_name: string, handler: (event: { cwd: string }) => { skillPaths: string[] }) => handlers.push(handler)),
  } as never);
...
describe("Policy R6 effect-typescript skill", () => {
  it("bridge 暴露四态 mode，未计算时为 undefined", () => {
    ...
    for (const mode of ["disabled", "respect-project", "prefer", "required"] as const) {
  ...
  it("仅 prefer/required 收集内置 skill", () => {
    ...
    try {
      ...
      for (const mode of ["prefer", "required"] as const) {
      ...
      for (const mode of ["disabled", "respect-project"] as const) {
        setPolicySection(undefined, mode);
  ...
  it("SKILL frontmatter、六种模式标题与引用路径均有效", () => {
    ...
    for (const path of paths) assert.ok(existsSync(join(process.cwd(), path)), path);
  });
});
