...
import {
  ...
  renderContextLines,
  renderContextText,
} from "../../extensions/hpl-context-viewer/renderer.js";
import type { ContextSnapshot } from "../../extensions/hpl-context-viewer/types.js";
...
describe("renderContextLines", () => {
  ...
  it("正常路径: 渲染进度条 + 表格", () => {
    ...
    assert.ok(lines.some((l: string) => l.includes("Estimated usage")), "含 Estimated usage");
    assert.ok(lines.some((l: string) => l.includes("20.0k / 128.0k")), "含总量统计");
  });

  it("正常路径: 表格包含所有 category", () => {
    const lines = renderContextLines(sample);
    assert.ok(lines.some((l: string) => l.includes("System prompt")), "System prompt");
    assert.ok(lines.some((l: string) => l.includes("System tools")), "System tools");
    assert.ok(lines.some((l: string) => l.includes("Free space")), "Free space");
  });

  it("正常路径: tool 名去除 _ 前缀", () => {
    const lines = renderContextLines(sample);
    assert.ok(lines.some((l: string) => l.includes("read") && !l.includes("_read")), "read 无 _ 前缀");
    assert.ok(lines.some((l: string) => l.includes("bash") && !l.includes("_bash")), "bash 无 _ 前缀");
  });

  it("正常路径: 包含 skill details", () => {
    const lines = renderContextLines(sample);
    assert.ok(lines.some((l: string) => l.includes("my-skill")), "skill name");
  });
});

describe("renderContextText", () => {
  it("正常路径: 返回字符串含换行", () => {
    const s: ContextSnapshot = {
