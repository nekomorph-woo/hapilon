...
/**
 * npm 扩展接线（prototype for #36）：
 *
 * hapilon 通过 `-e` 把 node_modules 里的第三方 pi 扩展注入 pi 内核，
 * 与 hpl-* 内置扩展同一加载通道。不走 `pi install`——那会写
 * `~/.hapilon/settings.json`，导致 subagent session 重新发现并激活这些扩展
 * （pi-subagents 明确要求 subagent 不激活自身）。
 *
...
export function resolveNpmExtensionPaths(): string[] {
  // 从本模块位置解析（dist/extensions.js），锚定 hapilon 的 node_modules
  const require = createRequire(import.meta.url);

  return NPM_EXTENSIONS.map(([pkg, entry]) => {
    const path = require.resolve(`${pkg}/package.json`).replace(
      /package\.json$/,
      entry,
    );
    if (!existsSync(path)) {
