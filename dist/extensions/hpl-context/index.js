...
export default function hplContext(pi) {
    ...
    pi.on("resources_discover", (event) => {
        const skillPaths = Effect.runSync(Effect.flatMap(collectUpwardEffect(event.cwd, userHome, "agents/skills"), (dirs) => Effect.map(Effect.forEach(dirs, (dir) => discoverSkillPathsEffect([dir])), (paths) => paths.flat())));
