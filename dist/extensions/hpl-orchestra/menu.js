...
import { currentRole, deleteTeamStateEffect, findTeamStateForPane, isTeamOwner, readTeamStateEffect, resolveSessionStatePath, writeTeamStateEffect, findRoleEntry, } from "./state.js";
...
function replaceFirstInstance(roles, key, instance) {
    const index = roles.findIndex((entry) => entry.key === key);
    if (index < 0)
        return [...roles, { key, instances: [instance] }];
...
function makeState(ownerPaneId, worker, reviewer, previous) {
    ...
    if (reviewer)
        roles = replaceFirstInstance(roles, "reviewer", reviewer);
    return {
        enabled: true,
...
export async function updateTeamStatus(ctx, spawn = defaultSpawn) {
    ...
    const segments = state.roles
        .filter((entry) => entry.instances.length > 0)
        .map((entry) => `${roleLabel(entry.key)} ${entry.instances.map((instance) => `${instance.paneId} ${probePaneLive(instance.paneId, spawn) ? "✓" : "✗"}`).join(" ")}`);
