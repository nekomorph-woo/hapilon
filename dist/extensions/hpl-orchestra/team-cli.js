...
function paneLines(key, paneId, nickname, spawn) {
    ...
    if (tasks.length === 0)
        return [header, "  tasks: 空（该 pane 还没建过任务）"];
 ...
 * 只读——绝不写团队状态、绝不写任务列表。
 */
export function runTeamStatusCommand(_args, spawn = defaultSpawn) {
 ...
 */
export function runTeamEnqueueCommand(args, now = Date.now) {
    ...
    const result = Effect.runSync(Effect.either(appendPendingTaskEffect(path, { paneId, subject, ...(brief ? { brief } : {}) }, now)));
    if (result._tag === "Left") {
 ...
 */
export function runWakeOwnerCommand(args, spawn = defaultSpawn) {
