...
export default function hplOrchestra(pi) {
    ...
    pi.on("session_before_switch", (event, ctx) => {
        if (event.reason !== "new")
            return;
        if (!process.env.HAPI_ORCH_ROLE || !process.env.HERDR_PANE_ID)
            return;
    ...
    pi.on("session_start", async (event, ctx) => {
        ...
        if (!saved || !process.env.HAPI_ORCH_ROLE || !process.env.HERDR_PANE_ID)
            return;
