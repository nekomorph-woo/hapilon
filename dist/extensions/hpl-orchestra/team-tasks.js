...
function parseStore(raw, path) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
        throw new Error(`任务列表结构不符（顶层不是对象）：${path}`);
    }
    const { nextId, tasks } = raw;
    if (!Array.isArray(tasks))
        throw new Error(`任务列表结构不符（缺 tasks 数组）：${path}`);
    ...
    return { nextId, tasks: tasks };
}
/** 读任务列表；文件不存在返回 undefined（新 pane 还没建过任务），形状不符则报错。 */
export const readTaskStoreEffect = (path) => Effect.try({
    try: () => {
        if (!existsSync(path))
            return undefined;
        return parseStore(JSON.parse(readFileSync(path, "utf8")), path);
    },
 ...
 */
export const appendPendingTaskEffect = (path, input, now = Date.now) => Effect.try({
    try: () => {
        const lockPath = `${path}.lock`;
        const token = acquireLock(lockPath);
        try {
            const store = existsSync(path)
                ? parseStore(JSON.parse(readFileSync(path, "utf8")), path)
                : { nextId: 1, tasks: [] };
...
export function briefDirOf(task) {
