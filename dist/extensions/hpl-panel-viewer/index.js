...
export default function hplPanelViewer(pi) {
    ...
    pi.on("session_start", (_event, ctx) => {
        ...
        ctx.ui.setWidget("hapi-pop", (tui, theme) => {
    ...
    pi.registerCommand("pop", {
        ...
        handler: async (args, ctx) => {
            ...
            if (pat && !target)
                ctx.ui.notify(`${POP_ICON} no panel matching "${pat}"`, "info");
    ...
    pi.registerTool({
        ...
        description: "Open the hapi-pop floating viewer showing a specific panel's content. " +
    ...
    pi.registerTool({
        ...
        label: "Pop config",
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        parameters: { type: "object", properties: { action: { type: "string" }, pattern: { type: "string" } }, required: ["action"] },
        description: "Configure hapi-pop viewer. Actions: 'show' <pattern>, 'hide' <pattern>, " +
