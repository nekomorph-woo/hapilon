...
export function dingLabel(percent) {
    if (percent === null)
        return "[HOT]";
    if (percent >= 95)
        return "[HOT!!!!!]";
...
// ─── 渐变色 ─────────────────────────────────────────────────────────
const DARK_YELLOW = [64, 52, 0];
const YELLOW = [255, 200, 0];
const RED = [220, 40, 30];
const DARK_RED = [139, 0, 0];
/** 黑字（配亮背景）/ 白字（配暗背景） */
const FG_DARK = [30, 30, 30];
const FG_LIGHT = [245, 245, 245];
 ...
 *   > 95     深红恒定
 */
export function dingColor(percent) {
...
export function renderDing(percent) {
