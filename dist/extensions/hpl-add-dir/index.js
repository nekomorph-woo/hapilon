...
import { scanDirContext, buildContextInjection, invalidateContextCache, resolveDir, dirExists, } from "./context.js";
...
// 扩展
// ---------------------------------------------------------------------------
export default function hplAddDir(pi) {
    // 会话级状态
    let addedDirs = [];
    // -----------------------------------------------------------------------
    // 状态重建
    ...
    function resolveInputPath(input, cwd) {
        // 若解析为已存在的目录，直接使用
        if (dirExists(resolveDir(input, cwd)))
            return input;
    ...
    function addDir(dirPath, cwd, ctx) {
        ...
        const found = [];
        if (dirCtx.hapilonMd)
            found.push("HAPILON.md");
        // 检测扩展并构建提示
        const extensionHints = [];
        if (dirCtx.extensionPaths.length > 0) {
            extensionHints.push(`Found ${dirCtx.extensionPaths.length} extension(s) in ${label}/.pi/extensions/.`, `   To enable them, add to your settings.json:`, `   { "extensions": ["${absolutePath}/.pi/extensions"] }`, `   Then /reload to activate.`);
        }
    ...
    function removeDir(absolutePath, ctx) {
