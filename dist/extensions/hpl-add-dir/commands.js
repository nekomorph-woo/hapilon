...
// ─── 命令注册 ─────────────────────────────────────────────────────────
export function registerCommands(pi, ops) {
    ...
    pi.registerCommand("dirs", {
        ...
        handler: async (_args, ctx) => {
            ...
            const lines = [`External directories (${dirs.length}):\n`];
            for (const dir of dirs) {
                const dirCtx = scanDirContext(dir.absolutePath);
                const badges = [];
                if (dirCtx.hapilonMd)
                    badges.push("HAPILON.md");
                if (dirCtx.extensionPaths.length > 0)
                    badges.push(`${dirCtx.extensionPaths.length} extension(s)`);
