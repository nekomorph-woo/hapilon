...
// ── Collector ──────────────────────────────────────────────────────────
/** 从各数据源聚合上下文快照（纯函数） */
export function collectContextSnapshot(input) {
    ...
    const toolsTotal = selectedTools.length > 0
        ...
        : null;
    const toolItems = selectedTools.map((name) => ({
        name,
    ...
    const contextFilesTokens = contextFiles.length > 0
        ...
        : null;
    const contextFileItems = contextFiles.map((f) => ({
        name: f.path,
    ...
    const staticTotal = (spTokens ?? 0) + (hapilonMdTokens ?? 0) + (rulesTokens ?? 0) +
        (toolsTotal ?? 0) + (contextFilesTokens ?? 0) + (skillsTokens ?? 0);
    const messagesTokens = tokens !== null && tokens > staticTotal ? tokens - staticTotal : null;
    // ── Build categories ───────────────────────────────────────
    const categories = [];
