...
export function parseAllowArgs(argsStr) {
    const arg = argsStr.trim();
    if (arg === "--list")
        return { kind: "list", paths: [] };
    if (arg === "--clear")
        return { kind: "clear", paths: [] };
    ...
    pi.on("tool_call", async (event, ctx) => {
        if (isToolCallEventType("write", event) || isToolCallEventType("edit", event)) {
            ...
            if (verdict === "confirm") {
                ...
                if (isTrusted(toolName, filePath, ctx.cwd))
                    return;
                const result = await requestConfirm(ctx, "⚠️ 写入确认", `Agent 正在尝试写入受保护文件：\n\n> ${filePath}\n\n类型：中危路径\n\n是否允许？`);
        ...
        if (isToolCallEventType("read", event)) {
            ...
            const filePath = resolveTarget(rawPath, ctx.cwd);
            if (isTrusted("read", filePath, ctx.cwd))
                return;
        ...
        handler: async (argsStr, ctx) => {
            ...
            if (args.kind === "list") {
                ...
                if (session.length > 0) {
                    ...
                    for (const s of session)
                        lines.push(`  ${s.toolName}: ${s.targets.join(", ")}`);
                }
                if (project.length > 0) {
                    lines.push("项目白名单：");
                    for (const p of project)
                        lines.push(`  ${p.toolName}: ${p.targets.join(", ")}`);
                }
                if (lines.length === 0) {
                    notify("白名单为空", "info");
                }
            ...
            for (const p of args.paths) {
                ...
                if (classifyPath(resolved, "write", ctx.cwd) === "block") {
                    const ok = await requestHighRiskConfirm(ctx, "⚠️⚠️ 高危路径确认", `将解除高危路径的写保护：\n\n> ${p}\n\n` +
