...
export const WRITE_BLOCK = [
    ...
    // SSH 密钥与授权
    {
        test: (_r, name) => ["id_rsa", "id_rsa.pub", "id_ed25519", "id_ed25519.pub",
    ...
    {
        test: (_r, name) => name.endsWith(".keystore") || name.endsWith(".truststore"),
        label: "Java 密钥库",
    },
];
// ─── 写保护 — 中危 confirm ──────────────────────────────────────
export const WRITE_CONFIRM = [
    // 环境变量文件
...
export const READ_CONFIRM = [
    ...
    { test: (r) => r === homedir() + "/.npmrc", label: "npm token" },
    // 环境变量文件：secret 只该被应用运行时读取；
    // agent 读了就进 LLM 上下文/transcript/API 日志。主会话 confirm，
    // subagent 在 index.ts 另行 block。.env.example 例外（模板无 secret）。
    {
        test: (r, name = basename(r)) => /^\.env/.test(name) && !/^\.env\.example$/.test(name) && !/^\.env\.sample$/.test(name),
        label: "env 文件",
    },
];
