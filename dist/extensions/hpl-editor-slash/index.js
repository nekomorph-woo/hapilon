...
class HapiSlashEditor extends CustomEditor {
    ...
    render(width) {
        ...
        if (!this.slashColorize)
            return lines;
        // 结构:[顶边框, ...正文行, 底边框, ...补全下拉行]。
        // 正文行数由内部计数器给出;字段是 private,读不到时(如 pi 升级改名)
        // 退化为不染色——功能关闭,不出错。
        const count = this.renderedVisibleLineCount;
...
export default function hplEditorSlash(pi) {
    const shared = { names: new Set() };
    ...
    const setup = (ctx) => {
        ...
        if (!autocompleteWrapped) {
            ...
            ctx.ui.addAutocompleteProvider((current) => {
                const provider = wrapAutocomplete(current);
                shared.names = extractNames(current.commands);
                return provider;
            });
        }
