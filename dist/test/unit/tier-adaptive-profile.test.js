...
import { adaptiveEventsPath, adaptiveProfilePath, appendAdaptiveEvent, deriveProfile, readAdaptiveEvents, readAdaptiveProfile, rebuildAdaptiveProfile, } from "../../extensions/hpl-model-tiers/adaptive-events.js";
import { readTierAdaptiveConfig, setTierAdaptiveEnabled, setTierAdaptiveSessionOverride, tierAdaptiveEnabled, fillLearnedThinking, planTierSelection, resolveExplicitModel, } from "../../extensions/hpl-model-tiers/adaptive.js";
...
after(() => {
    ...
    if (originalEnv.cache === undefined)
        ...
        process.env["HAPILON_QUOTA_CACHE"] = originalEnv.cache;
...
const event = (overrides) => overrides;
/** 三个 sonnet 候选：配置顺序 zai > deepseek > openai-codex */
function writeResolvedTiers() {
...
const snapshot = (provider, percent, now) => ({
    provider,
...
describe("planTierSelection 装配", () => {
    ...
    it("thinking 画像标签：4 条观察中，5 条出具体 level", () => {
        ...
        const labelsOf = (key) => planTierSelection({ tier: "sonnet", now: Date.now() }).order.find((candidate) => candidate.key === key).labels;
        add(4);
...
describe("tier-adaptive 兼容读取", () => {
    it("点名解析：tier 指代取档位表 thinking，具体 id 原样采信，非法格式拒绝", () => {
        const resolved = {
            opus: [],
            sonnet: [{ provider: "zai", id: "glm-5.3", thinking: "high" }],
            haiku: [],
        };
