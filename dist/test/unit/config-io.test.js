...
describe("config-io", () => {
    ...
    describe("readHapilonConfig()", () => {
        ...
        it("仅有 defaultProvider 时正确解析", () => {
            writeFileSync(join(tmpBase, "config.json"), JSON.stringify({ defaultProvider: "openai" }) + "\n");
            const config = readHapilonConfig();
        ...
        it("JSON 语法错误时返回 {} + 打印警告（含错误详情）", () => {
            ...
            try {
                ...
                assert.ok(warnings.join(" ").includes("SyntaxError") ||
                    warnings.join(" ").includes("Unexpected"), "警告应包含错误详情");
            }
            finally {
        ...
        it("defaultProvider 不是字符串 → warn + 忽略", () => {
            writeFileSync(join(tmpBase, "config.json"), JSON.stringify({ defaultProvider: 42, defaultModel: "valid" }) + "\n");
            const warnings = [];
            const originalWarn = console.warn;
            console.warn = (...args) => {
                warnings.push(args.map(String).join(" "));
        ...
        it("正确解析 safetyNoticeShown 为 true", () => {
            writeFileSync(join(tmpBase, "config.json"), JSON.stringify({ safetyNoticeShown: true, defaultProvider: "test" }) + "\n");
            const config = readHapilonConfig();
        ...
        it("safetyNoticeShown 不是布尔值 → warn + 忽略", () => {
            writeFileSync(join(tmpBase, "config.json"), JSON.stringify({ safetyNoticeShown: "yes" }) + "\n");
            const warnings = [];
            const originalWarn = console.warn;
            console.warn = (...args) => {
                warnings.push(args.map(String).join(" "));
        ...
        it("正确解析 safetyNoticeShown 为 false", () => {
            writeFileSync(join(tmpBase, "config.json"), JSON.stringify({ safetyNoticeShown: false }) + "\n");
            const config = readHapilonConfig();
            assert.strictEqual(config.safetyNoticeShown, false);
        });
        it("safetyNoticeShown 为 null → warn + 忽略", () => {
            writeFileSync(join(tmpBase, "config.json"), JSON.stringify({ safetyNoticeShown: null }) + "\n");
            const warnings = [];
            const originalWarn = console.warn;
            console.warn = (...args) => {
                warnings.push(args.map(String).join(" "));
    ...
    describe("stripHapilonFlags()（#38）", () => {
        ...
        it("剥离 --sandbox=value 形式", () => {
            assert.deepStrictEqual(stripHapilonFlags(["--sandbox=full", "-p", "hi"]), ["-p", "hi"]);
        });
        it("两个自有 flag 都剥离，其余原样保留顺序", () => {
            assert.deepStrictEqual(stripHapilonFlags(["--model", "gpt-4o", "--sandbox", "--no-safety", "--mode", "json"]), ["--model", "gpt-4o", "--mode", "json"]);
        });
        it("无自有 flag 时原样返回", () => {
