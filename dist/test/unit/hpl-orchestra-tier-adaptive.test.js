...
/** 只统计 pane run 的启动命令 */
function paneRunCommands(calls) {
...
describe("orchestra 非 Worker 角色显式点名", () => {
    ...
    function addThinkingSamples(role, model, level) {
        const ts = new Date().toISOString();
        for (let i = 0; i < 5; i++) {
            appendAdaptiveEvent({
...
describe("角色 pane 主动切模事实上报", () => {
    ...
    it("扩展注册 model_select 监听并走同一条上报路径", () => {
        ...
        try {
            ...
            handler({ type: "model_select", source: "set", model: { provider: "zai", id: "glm-5.3" }, previousModel: { provider: "anthropic", id: "claude-opus" } }, { thinkingLevel: "high" });
            const [recorded] = readAdaptiveEvents();
...
describe("角色 pane 主动切 thinking level 上报", () => {
    ...
    it("直接切 thinking：缓冲一拍后落盘到当前模型", async () => {
        process.env.HAPI_ORCH_ROLE = "worker";
        try {
            assert.equal(recordThinkingSwitch({ level: "high", previousLevel: "medium" }, { model: { provider: "zai", id: "glm-5.3" } }), true);
    ...
    it("owner/缺当前模型/同 level 一律不记", () => {
        ...
        try {
            ...
            assert.equal(recordThinkingSwitch({ level: "high", previousLevel: "high" }, ctx), false, "同 level pi 本就不发事件，写侧兼底");
        }
        finally {
            delete process.env.HAPI_ORCH_ROLE;
        }
    ...
    it("扩展接线：thinking_level_select 经缓冲落盘，同轮 model_select 丢弃附带变化", async () => {
        ...
        try {
            ...
            assert.ok(model);
            thinking({ type: "thinking_level_select", level: "high", previousLevel: "medium" }, { model: { provider: "zai", id: "glm-5.3" } });
            await flush();
            ...
            thinking({ type: "thinking_level_select", level: "low", previousLevel: "high" }, { model: { provider: "zai", id: "glm-5.3" } });
