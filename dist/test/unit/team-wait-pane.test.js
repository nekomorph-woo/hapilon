...
import { DEFAULT_TIMEOUT_SEC, WAIT_EXIT, runWaitPaneCommand, waitForPaneSettle, } from "../../extensions/hpl-orchestra/wait-pane.js";
/**
 * 脚本化依赖：snapshot 按序吐状态（超出序列则停在最后一个），now 由 sleepMs 推进，
 * 于是超时判定不依赖真实时间、也不会真的挂住测试。
 */
function scriptedDeps(script) {
    let now = 0;
    const deps = {
        calls: 0,
        snapshot: () => {
...
const idle = (seq) => ({ status: "idle", seq });
const working = (seq) => ({ status: "working", seq });
