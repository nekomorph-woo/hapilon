...
describe("hpl-model-tiers /tiers 命令", { concurrency: false }, () => {
    ...
    it("「设置 thinking」给条目追加后缀、可覆盖、可清除", async () => {
        ...
        assert.deepEqual(JSON.parse(readFileSync(join(home, "model-tiers.json"), "utf8")).sonnet, ["glm-a:high", "glm-b"]);
        ...
        assert.deepEqual(JSON.parse(readFileSync(join(home, "model-tiers.json"), "utf8")).sonnet, ["glm-a:max", "glm-b"]);
        // 清除：选「清除（跟随全局默认）」移除后缀
        ...
        assert.deepEqual(JSON.parse(readFileSync(join(home, "model-tiers.json"), "utf8")).sonnet, ["glm-a", "glm-b"]);
    });
