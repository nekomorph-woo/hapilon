...
const sandbox = (cmd) => checkSandboxWrite(cmd, { ...SANDBOX_OPTS, home: testHome });
...
const readAudit = () => existsSync(auditPath())
...
describe("hpl-safety-gate auto", () => {
    before(() => {
        ...
        writeFileSync(join(testHome, "model-tiers-resolved.json"), JSON.stringify({
            opus: [],
    ...
    describe("judgeCommand()（mock complete）", () => {
        ...
        const run = (d) => Effect.runPromise(Effect.either(judgeCommand(d)));
        it("判定 allow → Right 且带具体模型", async () => {
            const result = await run(deps());
            assert.equal(result._tag, "Right");
            if (result._tag === "Right") {
                assert.deepEqual({ verdict: result.right.verdict, reason: result.right.reason, model: result.right.model }, { verdict: "allow", reason: "常规工作流", model: "zai/glm-4.7" });
            }
        });
        it("围栏 JSON 也能解析", async () => {
            const result = await run(deps({ complete: completeWith('```json\n{"verdict":"block","reason":"删除生产数据"}\n```') }));
            assert.equal(result._tag, "Right");
            if (result._tag === "Right")
                assert.equal(result.right.verdict, "block");
        });
        it("非法 JSON → GateAutoInvalidOutput", async () => {
            const result = await run(deps({ complete: completeWith("好的，我认为可以") }));
            assert.equal(result._tag, "Left");
            if (result._tag === "Left")
                assert.equal(result.left._tag, "GateAutoInvalidOutput");
        });
        it("verdict 越界值 → GateAutoInvalidOutput（schema 校验）", async () => {
            const result = await run(deps({ complete: completeWith('{"verdict":"maybe","reason":"嗯"}') }));
            assert.equal(result._tag, "Left");
            if (result._tag === "Left")
                assert.equal(result.left._tag, "GateAutoInvalidOutput");
        });
        it("complete 拒绝 → GateAutoApiError", async () => {
            const result = await run(deps({ complete: () => Promise.reject(new Error("boom")) }));
            assert.equal(result._tag, "Left");
            if (result._tag === "Left")
                assert.equal(result.left._tag, "GateAutoApiError");
        });
        it("超时 → GateAutoTimeout", async () => {
            const result = await run(deps({ timeoutMs: 30, complete: () => new Promise(() => { }) }));
            assert.equal(result._tag, "Left");
            if (result._tag === "Left")
                assert.equal(result.left._tag, "GateAutoTimeout");
        });
        ...
        it("glob 模型指代直接匹配可用列表", async () => {
            ...
            assert.equal(result._tag, "Right");
            if (result._tag === "Right")
                assert.equal(result.right.model, "zai/glm-4.7");
        });
    });
    describe("108 样本重放", () => {
    ...
    describe("handler 级兜底与缓存（无 UI 上下文）", () => {
        ...
        it("模型超时 → 回落硬阻止 + 审计 fallback-block（reason 含超时）", async () => {
            ...
            writeFileSync(join(testHome, "agent", "settings.json"), JSON.stringify({ gateAuto: { enabled: true, timeoutMs: 30 } }));
            try {
                const result = await call("git push origin main");
                assert.ok(result?.block);
                const last = readAudit().at(-1);
