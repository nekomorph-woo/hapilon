...
describe("hpl-protected-paths", () => {
    ...
    describe("resolveTarget()", () => {
        ...
        it("绝对路径 → 解析 symlink", () => {
            ...
            assert.ok(r.endsWith("/etc/hosts") || r.endsWith("/private/etc/hosts"), "应解析为 /etc/hosts 或其 symlink 真实路径");
        });
        it("空路径 → cwd", () => {
    ...
    describe("拦截日志（tool_call 回调）", () => {
        ...
        it("write BLOCK 命中 → console.warn 记录 reason（不再静默）", async () => {
            ...
            try {
                const result = (await handler(writeEvent("~/.ssh/id_rsa"), noUI));
        ...
        it("write CONFIRM 非交互拒绝 → console.warn 记录 reason", async () => {
            ...
            const warn = mock.method(console, "warn");
            try {
                const result = (await handler(writeEvent(".env"), noUI));
                assert.strictEqual(result?.block, true);
                assert.strictEqual(warn.mock.callCount(), 1);
        ...
        it("read CONFIRM 非交互拒绝 → console.warn 记录 reason", async () => {
            ...
            const warn = mock.method(console, "warn");
            try {
                const result = (await handler(readEvent("~/.ssh/id_rsa"), noUI));
                assert.strictEqual(result?.block, true);
                assert.strictEqual(warn.mock.callCount(), 1);
