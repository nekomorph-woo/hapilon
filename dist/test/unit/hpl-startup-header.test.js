...
describe("logo 主题渲染", () => {
    it("盲文点阵 logo 行使用 accent 主题槽位", () => {
        ...
        const component = createStartupHeader({ cwd: "/tmp/project" }, {}, theme, { expanded: false });
        const rendered = component.render(80);
        assert.ok(rendered.some((line) => line.startsWith("<accent>") && /[\u2800-\u28FF]/.test(line)), "logo 行应以 accent 渲染");
    ...
    it("扩展名列表使用 muted，ctrl+o 提示仍使用 dim", () => {
        ...
        const extensionComponent = createStartupHeader({ cwd: "/tmp/project" }, {}, theme, { expanded: false });
        ...
        try {
            ...
            if (original === undefined)
                delete process.env.HAPILON_EXTENSIONS;
            else
                process.env.HAPILON_EXTENSIONS = original;
        }
    });
    it("宽屏扩展名 muted 不会吞掉左列主体颜色", () => {
        ...
        try {
            createStartupHeader({ model: { provider: "openai", name: "gpt-4o" }, cwd: "/tmp/project" }, {}, theme, { expanded: false }).render(80);
