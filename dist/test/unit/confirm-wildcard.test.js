...
describe("requestConfirm() — 通配选项", () => {
    it("无 allowSuggestion → 维持原 4 选项，无通配入口", async () => {
        ...
        const result = await requestConfirm(ctx, "T", "M");
    ...
    it("有 allowSuggestion → 增出通配入口（会话/项目各一）", async () => {
        ...
        await requestConfirm(ctx, "T", "M", { allowSuggestion: "git push*" });
        assert.deepStrictEqual(calls.select[0]?.options, [
            "Allow Once",
            "Allow this Session",
            "Allow this Project",
    ...
    it("选通配（会话）+ 输入自定义模式 → addTrust 应收到该模式", async () => {
        ...
        const result = await requestConfirm(ctx, "T", "M", { allowSuggestion: "git push*" });
    ...
    it("选通配（项目）→ scope project", async () => {
        ...
        const result = await requestConfirm(ctx, "T", "M", { allowSuggestion: "npm install*" });
        assert.deepStrictEqual(result, {
            status: "approved",
            scope: "project",
