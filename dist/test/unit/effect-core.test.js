...
import { ensureQuietStartupEffect, writeAuthFileNativeEffect, writeSkeletonFilesEffect, } from "../../providers/providers.js";
...
describe("Effect 核心 API", () => {
    ...
    it("writeHapilonConfigEffect 失败走 Fail 通道且可被 catchTag 捕获", () => {
        ...
        const caught = Effect.runSync(writeHapilonConfigEffect({}).pipe(Effect.catchTag("ConfigWriteError", () => Effect.succeed("caught"))));
    ...
    it("ensureHapilonDirsEffect 失败走 Fail 通道且可被 catchTag 捕获", () => {
        ...
        if (exit._tag === "Failure")
            assert.equal(exit.cause._tag, "Fail");
        const caught = Effect.runSync(ensureHapilonDirsEffect.pipe(Effect.catchTag("HapilonHomeError", () => Effect.succeed("caught"))));
        assert.equal(caught, "caught");
        process.env.HAPILON_HOME = tmpBase;
    });
    it("providers 的写入 Effects 失败走 AuthWriteError Fail 通道", () => {
        ...
        for (const effect of effects) {
            ...
            if (exit._tag === "Failure")
                assert.equal(exit.cause._tag, "Fail");
            assert.equal(Effect.runSync(effect.pipe(Effect.catchTag("AuthWriteError", () => Effect.succeed("caught")))), "caught");
        }
    });
    it("ensureExtensionConfigsEffect 失败走 ConfigWriteError Fail 通道", () => {
        ...
        if (exit._tag === "Failure")
            assert.equal(exit.cause._tag, "Fail");
        assert.equal(Effect.runSync(ensureExtensionConfigsEffect(blocked).pipe(Effect.catchTag("ConfigWriteError", () => Effect.succeed("caught")))), "caught");
    });
    ...
    it("removeSafetyExtensionsEffect 写入失败走 Fail 通道", () => {
        ...
        if (exit._tag === "Failure")
            assert.equal(exit.cause._tag, "Fail");
        assert.equal(Effect.runSync(removeSafetyExtensionsEffect(blocked).pipe(Effect.catchTag("SafetySettingsError", () => Effect.succeed("caught")))), "caught");
    });
    it("MCP 校验失败走 McpConfigError Fail 通道且同步包装保留实例类型", () => {
        const def = { name: "bad/name", type: "stdio" };
        const exit = Effect.runSyncExit(addMcpServerEffect(tmpBase, def));
        assert.equal(exit._tag, "Failure");
        if (exit._tag === "Failure")
            assert.equal(exit.cause._tag, "Fail");
        assert.equal(Effect.runSync(addMcpServerEffect(tmpBase, def).pipe(Effect.catchTag("McpConfigError", () => Effect.succeed("caught")))), "caught");
    ...
    it("listModelsForProviderEffect 非法 HAPILON_HOME 走 Fail 通道", async () => {
        ...
        if (exit._tag === "Failure")
            assert.equal(exit.cause._tag, "Fail");
        await assert.rejects(listModelsForProvider("openai"), (err) => {
    ...
    it("Effect.async 中断后迟到的 resume 不产生 unhandled rejection", async () => {
        ...
        try {
            ...
            const result = await Effect.runPromise(delayed.pipe(Effect.timeout("1 millis"), Effect.catchAll(() => Effect.succeed("interrupted"))));
    ...
    it("prepareStartupEffect：角色 pane 的任务列表落成 PI_TASKS，owner 与非 role 不注入", async () => {
        ...
        try {
            ...
            if (savedRole === undefined)
                ...
                process.env.HAPI_ORCH_ROLE = savedRole;
            if (savedTasks === undefined)
                delete process.env.PI_TASKS;
            else
                process.env.PI_TASKS = savedTasks;
        }
    });
    it("--team-tasks 必须是绝对路径，且与 --team-role 同用", async () => {
        await assert.rejects(() => Effect.runPromise(prepareStartupEffect(["--team-role", "worker", "--team-tasks", "rel.json"])), (error) => /绝对路径/.test(error.message));
        await assert.rejects(() => Effect.runPromise(prepareStartupEffect(["--team-tasks", "/tmp/x.tasks.json"])), (error) => /--team-role/.test(error.message));
    });
    it("readHapilonMdEffect 读取失败走 ReadHapilonMdError Fail 通道", () => {
        ...
        if (exit._tag === "Failure")
            assert.equal(exit.cause._tag, "Fail");
        assert.equal(Effect.runSync(readHapilonMdEffect([missing]).pipe(Effect.catchTag("ReadHapilonMdError", () => Effect.succeed("caught")))), "caught");
    });
    ...
    it("hpl-econ storeFullOutputEffect 失败走 EconStoreError Fail 通道", () => {
        ...
        if (exit._tag === "Failure")
            assert.equal(exit.cause._tag, "Fail");
        assert.equal(Effect.runSync(storeFullOutputEffect(blocked, "ref", "output").pipe(Effect.catchTag("EconStoreError", () => Effect.succeed("caught")))), "caught");
    });
