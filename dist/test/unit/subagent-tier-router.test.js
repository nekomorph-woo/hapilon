...
describe("subagent tier-router", { concurrency: false }, () => {
    ...
    const opus = { provider: "anthropic", id: "claude-opus-4", reasoning: true };
    const sonnet = { provider: "anthropic", id: "claude-sonnet-4" };
    const haiku = { provider: "anthropic", id: "claude-haiku-3" };
    ...
    function writeResolved(overrides = {}) {
    ...
    function context(complete = async () => ({ content: [{ type: "text", text: "sonnet" }] })) {
        return {
            cwd: project,
    ...
    it("explicit 档位最高优先，跳过映射和分类器", async () => {
        ...
        let calls = 0;
        const result = await Effect.runPromise(routeTierEffect(request({ explicitTier: "haiku", taskType: "review" }), context(async () => {
            calls++;
    ...
    it("映射未命中时用 haiku 分类器，并接受大小写档名", async () => {
        for (const answer of ["haiku", "SONNET", "opus"]) {
            const result = await Effect.runPromise(routeTierEffect(request({ taskType: "unmapped" }), context(async (_model, _context, options) => {
    ...
    it("分类器抛错或超时后 fallback 到 sonnet", async () => {
        const failed = await Effect.runPromise(routeTierEffect(request(), context(async () => {
        ...
        try {
            const timedOut = await Effect.runPromise(routeTierEffect(request(), context(async (_model, _context, options) => await new Promise((_resolve, reject) => {
