...
describe("内置扩展全集加载", () => {
    ...
    it("同名 command / flag 只由一个扩展注册", () => {
        const duplicates = [];
        for (const kind of ["command", "flag"]) {
            const owners = new Map();
            for (const ext of result.extensions) {
                const names = kind === "command" ? ext.commands.keys() : ext.flags.keys();
                for (const name of names) {
                    const owner = owners.get(name);
                    if (owner && owner !== ext.path)
                        duplicates.push(`${kind} ${name}: ${owner} + ${ext.path}`);
                    else
    ...
    it("关键扩展确实在加载集合里", () => {
        ...
        for (const name of ["hpl-model-tiers", "hpl-orchestra", "hpl-quota-usage"]) {
            assert.ok(loaded.some((path) => path.endsWith(join(name, "index.js"))), `${name} 未加载`);
        }
    });
});
