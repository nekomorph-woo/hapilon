...
import { formatTokens, formatWindow, buildLine1, buildModelRight, buildStatusLine, aggregateUsage, buildStatsLeft, visibleWidth, layoutLineRight, shortenHome, truncatePlain, } from "../../extensions/hpl-footer/format.js";
    ...
    it("truncatePlain 在任意宽度下都不超宽（用 pi-tui 的表复核）", () => {
        for (const text of samples) {
            for (let width = 1; width <= piTuiVisibleWidth(text) + 2; width++) {
                assert.ok(piTuiVisibleWidth(truncatePlain(text, width)) <= width, `超宽: text=${text} width=${width}`);
            }
        }
    });
});
