...
import { ROLE_TEXT, CUSTOM_TOOLS_NOTE, buildPiDocText, BUILTIN_GUIDELINES, COMMIT_DISCIPLINE_TEXT, DISPATCH_DISCIPLINE_TEXT, ROLE_COMMIT_BOUNDARY_TEXT, } from "../../extensions/hpl-system-prompt/sections.js";
import { buildRoleSection, buildCodeStyleSection, buildCommitDisciplineSection, buildDispatchDisciplineSection, buildRoleCommitBoundarySection, buildToolsSection, buildCustomToolsNote, buildGuidelinesSection, buildPiDocSection, buildHapilonInstructions, buildHapilonRules, buildContextSection, buildSkillsSection, buildAppendSection, buildExternalDirsSection, buildEnvironmentSection, assembleSystemPrompt, collectHapilonContext, } from "../../extensions/hpl-system-prompt/assemble.js";
...
describe("buildGuidelinesSection", () => {
    it("正常路径: 组合工具级和内建 guidelines", () => {
        const result = buildGuidelinesSection(["Use read to examine files instead of cat or sed."], ["read", "bash", "edit", "write"]);
    ...
    it("正常路径: 重复与空白 guideline 被去重和过滤", () => {
        const result = buildGuidelinesSection(["Be concise in your responses", "  Be concise in your responses  ", ""], ["read"]);
        assert.equal((result.match(/Be concise in your responses/g) ?? []).length, 1, "重复条目仅出现一次");
    });
...
describe("buildHapilonInstructions", () => {
    ...
    it("异常路径: 正文含 </hapilon_instructions> 字面量时结构不被破坏", () => {
        ...
        assert.equal((result.match(/<\/hapilon_instructions>/g) ?? []).length, 1, "仅结尾一个闭合标签");
    });
    it("边界条件: 空数组返回空字符串", () => {
...
describe("buildEnvironmentSection", () => {
    ...
    it("#50 提供 agentDirPath 时注入 MCP 环境段（配置路径 + schema 摘要）", () => {
        ...
        assert.ok(result.includes("/home/user/.hapilon/agent/mcp.json"), "写明 mcp.json 绝对路径（agent 才不会猜 ~/.pi）");
    ...
    it("#50 MCP 段 token 成本上界（<150 token ≈ 600 chars）", () => {
        ...
        assert.ok(mcpPart.length < 600, `MCP 段 ${mcpPart.length} chars 超 600（约 150 token）上界`);
    });
});
...
describe("assembleSystemPrompt", () => {
    ...
    it("正常路径: 全量 section（含 contextFiles/skills/append）与顺序断言", () => {
        ...
        assert.ok(result.indexOf("<code_style>") > result.indexOf("<guidelines>") &&
    ...
    it("#50 assemble 传入 agentDirPath 时 environment 含 MCP 段", () => {
        ...
        assert.ok(result.includes("/home/u/.hapilon/agent/mcp.json"), "MCP 配置路径进 final prompt");
    });
...
describe("hplSystemPrompt handler", () => {
    ...
    it("异常路径: 组装抛错时降级返回空对象且 console.error 被调用", () => {
        ...
        try {
            ...
            assert.ok(String(errors[0][0]).includes("[hpl-system-prompt]"), "错误日志含扩展标识");
        }
        finally {
