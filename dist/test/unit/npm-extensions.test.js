...
describe("resolveExtensionEntry()（fail fast 分支）", () => {
    const okResolve = (id) => "/fake/node_modules/" + id.replace("/", "+");
    it("入口文件存在时返回拼接路径（用真实包验证 join 语义）", () => {
        ...
        const path = resolveExtensionEntry("@tintinweb/pi-tasks", "dist/index.js", () => join(pkgDir, "package.json"));
        assert.equal(path, realEntry);
        assert.ok(existsSync(path));
    });
    it("入口文件缺失时抛错且消息含包名与入口", () => {
        assert.throws(() => resolveExtensionEntry("@tintinweb/pi-tasks", "dist/index.js", okResolve), (err) => err.message.includes("@tintinweb/pi-tasks") &&
        ...
        try {
            ...
            const resolve = (id) => {
                if (id === "pi-mcp-adapter/package.json") {
                    const err = new Error(`Package subpath './package.json' is not defined by "exports"`);
    ...
    it("resolve 抛错（包不存在）时错误向上传播", () => {
        assert.throws(() => resolveExtensionEntry("@ghost/pkg", "dist/index.js", () => {
