...
describe("providers", () => {
    ...
    describe("readAuthFile()", () => {
        ...
        it("条目是纯字符串（非 {type,key} 对象）→ warn + 跳过", () => {
            writeFileSync(join(tmpDir, "auth.json"), JSON.stringify({ deepseek: "sk-plain-string", openai: { type: "api_key", key: "sk-xxx" } }) + "\n");
            const warnings = [];
            const originalWarn = console.warn;
            console.warn = (...args) => {
                warnings.push(args.map(String).join(" "));
        ...
        it("条目缺少 key 字段 → warn + 跳过", () => {
            writeFileSync(join(tmpDir, "auth.json"), JSON.stringify({ bad: { type: "api_key" }, good: { type: "api_key", key: "sk-xxx" } }) + "\n");
            const warnings = [];
            const originalWarn = console.warn;
            console.warn = (...args) => {
                warnings.push(args.map(String).join(" "));
