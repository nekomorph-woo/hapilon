...
function configShow() {
    ...
    if (config.defaultProvider && config.defaultModel) {
        ...
        console.log(`默认 provider: ${config.defaultProvider}（模型未设置）`);
    }
...
// ─── config default ──────────────────────────────────────────────────
async function configSetDefaultInteractive() {
    if (!stdin.isTTY) {
    ...
    try {
        ...
        for (let i = 0; i < configuredList.length; i++) {
            console.log(`  ${i + 1}. ${configuredList[i].id.padEnd(16)}(${configuredList[i].name})`);
        }
        // 2. 选 provider
        const providerAnswer = (await question(rl, `\n选择默认 Provider [1-${configuredList.length}]: `)).trim();
        const providerIdx = Number.parseInt(providerAnswer, 10) - 1;
        if (isNaN(providerIdx) ||
            providerIdx < 0 ||
        ...
        const providerName = configuredList[providerIdx].name;
        ...
        for (let i = 0; i < models.length; i++) {
            console.log(`  ${i + 1}. ${models[i].model.padEnd(24)}(${models[i].context})`);
        }
        // 5. 选模型
        const modelAnswer = (await question(rl, `\n选择默认模型 [1-${models.length}]: `)).trim();
...
// ─── config provider add ─────────────────────────────────────────────
async function pickProviderFromList(rl, list, prompt) {
    console.log(`\n${prompt}:`);
    for (let i = 0; i < list.length; i++) {
        console.log(`  ${i + 1}. ${list[i].id.padEnd(20)}${list[i].name}`);
    }
    const answer = (await question(rl, `\n选择 [1-${list.length}]（留空取消）: `)).trim();
    if (!answer)
        return null;
...
async function configProviderAdd(targetId) {
    if (!stdin.isTTY) {
        console.error("错误: 此命令需要交互式终端");
        process.exit(1);
    }
    ...
    try {
        ...
        const key = (await question(rl, `输入 ${def.name} API Key（留空跳过）: `)).trim();
        if (!key) {
...
async function configProviderRemove(targetId) {
    if (!stdin.isTTY) {
        console.error("错误: 此命令需要交互式终端");
        process.exit(1);
    }
    ...
    try {
        ...
        const confirm = await yesno(rl, `${name} (${selectedId}) 将被删除。确认？`);
        if (!confirm) {
            console.log("已取消");
            return;
        }
