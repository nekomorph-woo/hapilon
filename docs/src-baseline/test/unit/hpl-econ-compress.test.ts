...
import {
  ...
  compressOutput,
  type EconParams,
} from "../../extensions/hpl-econ/compress.js";
...
describe("hpl-econ/compress", () => {
  ...
  const params = (over: Partial<EconParams> = {}): EconParams => ({
    threshold: 8192,
    headLines: 40,
    tailLines: 20,
    storeDir: store,
  ...
  describe("compressOutput()", () => {
    ...
    it("省略提示含内核 fullOutputPath（双层截断处理，#52 警告段）", () => {
      ...
      assert.ok(
        result.text.includes(kernelPath),
        "提示含内核落盘路径——>50KB 输出首段靠它找回",
      );
    });
    ...
    it(">50KB 场景：提示含「头部已被系统裁剪」如实告知", () => {
      ...
      assert.ok(
        result.text.includes("trimmed by the system"),
        "kernelTruncated 时提示说明头部可能不完整",
      );
    });
    ...
    it("压缩后体积显著小于原文", () => {
      const lines: string[] = [];
      for (let i = 0; i < 20000; i++) lines.push(`log entry number ${i} with some padding text`);
      const full = lines.join("\n");
    ...
    it("返回结构含统计字段（供 JSONL 记录）", () => {
      const lines: string[] = [];
      for (let i = 0; i < 3000; i++) lines.push(`entry-${i}`);
      const full = lines.join("\n");
