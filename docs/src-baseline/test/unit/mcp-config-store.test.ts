...
describe("mcp/config-store", () => {
  ...
  describe("loadMcpServers()", () => {
    ...
    it("读取既有 servers", () => {
      ...
      writeFileSync(
        join(dir, "mcp.json"),
        JSON.stringify({ mcpServers: { fs: { type: "stdio", command: "npx", args: ["-y", "x"] } } }),
      );
      const servers = loadMcpServers(dir);
      const cmd = (servers.fs as unknown as { command: string }).command;
  ...
  describe("addMcpServer()", () => {
    ...
    it("同名已存在时抛错（fail fast，不静默覆盖）", () => {
      ...
      assert.throws(
        () => addMcpServer(dir, { name: "a", type: "stdio", command: "y" }),
        (err: Error) => err.message.includes("a") && err.message.includes("已存在"),
      );
    });
    ...
    it("stdio 缺 command 抛错（schema 校验前置）", () => {
      ...
      mkdirSync(dir, { recursive: true });
      assert.throws(
        () => addMcpServer(dir, { name: "bad", type: "stdio" }),
        McpConfigError,
      );
    });
    ...
    it("http 缺 url 抛错", () => {
      ...
      mkdirSync(dir, { recursive: true });
      assert.throws(
        () => addMcpServer(dir, { name: "bad", type: "http" }),
        McpConfigError,
      );
    });
    ...
    it("非法 transport 类型抛错", () => {
      ...
      mkdirSync(dir, { recursive: true });
      assert.throws(
        () => addMcpServer(dir, { name: "bad", type: "carrier-pigeon", command: "x" }),
        McpConfigError,
      );
    });
  });
