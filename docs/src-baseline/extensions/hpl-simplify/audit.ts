...
export function parseScope(args: string): AuditScope | null {
  const trimmed = args.trim();
  if (trimmed === "") return { kind: "commit", ref: "HEAD" };
...
export function scopeDiffCommand(scope: AuditScope): string {
  switch (scope.kind) {
    case "staged":
...
export function buildAuditPrompt(scope: AuditScope): string {
  ...
  return (
    `/simplify audit (read-only). Audit ${scopeDesc}.\n\n` +
...
export function buildApplyPrompt(approved: number[]): string {
  const list = approved.join(", ");
  return (
    `/simplify apply. A previous /simplify audit produced a numbered findings report ` +
